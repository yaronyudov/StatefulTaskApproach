...
# 2) Details (MongoDB) by the matchId returned above
curl 'http://localhost:8082/matches/<matchId>'

# 3) Live deltas over SSE (open before re-triggering the scrapper / new events)
...
- OpenSearch holds **first-match rows only** (discovery); MongoDB holds **every delta** keyed by
  `_id = matchId` (details).
