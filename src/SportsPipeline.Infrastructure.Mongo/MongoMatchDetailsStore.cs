...
/// </summary>
public sealed class MongoMatchDetailsStore(IMongoCollection<BsonDocument> collection) : IMatchDetailsStore
{
    ...
    public async Task UpsertAsync(string matchId, string json, CancellationToken cancellationToken = default)
    {
        var filter = Builders<BsonDocument>.Filter.Eq("_id", matchId);
        var doc = BsonDocument.Parse(json);
        doc["_id"] = matchId; // Ensure id is set
