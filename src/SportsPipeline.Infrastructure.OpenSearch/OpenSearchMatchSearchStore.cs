...
/// </summary>
public sealed class OpenSearchMatchSearchStore(IOpenSearchClient client, string index) : IMatchSearchStore
{
    ...
    public async Task IndexAsync(string matchId, string json, CancellationToken cancellationToken = default)
    {
        var response = await client.LowLevel.IndexAsync<StringResponse>(index, matchId, PostData.String(json), ctx: cancellationToken);
        if (!response.Success)
        {
            throw new InvalidOperationException($"OpenSearch index failed: {response.DebugInformation}");
        }
    }
}
